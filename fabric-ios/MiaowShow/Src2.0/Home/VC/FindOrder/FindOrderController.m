...
@interface FindOrderController ()<UIScrollViewDelegate>
...
- (void)loadCate{
    ...
    [BAIRUITECH_NetWorkManager FinanceLiveShow_GetOderTypes:@{@"type":@"2"} withSuccessBlock:^(NSDictionary *object) {
...
- (void)addChildViewControllers:(NSArray *)ary{
    ...
    for (int i = 0; i < self.titleMutableArray.count; i++) {
        ...
        @weakify(self);
        FindOrderContentController * VC = [[FindOrderContentController alloc] init];
        VC.dic = ary[i];
